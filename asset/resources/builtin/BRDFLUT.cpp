...
[[nodiscard]] std::tuple<rhi::ImagePtr, rhi::ImageViewPtr> generateBRDFLUT(rhi::CommandBufferPtr cmdBuffer, rhi::DevicePtr device){
    ...
    rhi::ImageViewInfo viewInfo{
        ...
        .format = brdfLutImageInfo.format,
    };
    auto brdfImgView = rhi::ImageViewPtr(device->createImageView(viewInfo));
    ...
    pplInfo.vertexLayout = scene::Quad::vertexLayout;

    rhi::RenderPassInfo rpInfo{};
    auto& attachment = rpInfo.attachments.emplace_back();
    attachment.format = rhi::Format::RG16_SFLOAT;
    ...
    auto pplLayout = rhi::PipelineLayoutPtr(device->createPipelineLayout({}));

    std::string vsrc{"#version 450 core\n"};
    vsrc.append(brdfVertSrc);
    ...
    rhi::FrameBufferInfo fbInfo{
        ...
        .images = {brdfImgView.get()},
        .width = RESOLUTION,
        .height = RESOLUTION,
        .layers = 1,
    };
    auto frameBuffer = rhi::FrameBufferPtr(device->createFrameBuffer(fbInfo));
    ...
    renderEncoder->bindVertexBuffer(scene::Quad::vertexBuffer.buffer.get(), 0);
    renderEncoder->draw(scene::Quad::vertexCount, 1, 0, 0);
    ...
    return std::make_tuple(brdfImg, brdfImgView);
}
